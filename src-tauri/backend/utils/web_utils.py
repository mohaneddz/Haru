...
from constants import TRUSTED_DOMAINS, MAX_CONCURRENCY, USER_AGENT
...
# ProcessPoolExecutor for CPU-bound blocking calls (trafilatura.extract)
PROCESS_POOL = ProcessPoolExecutor(max_workers=max(1, (os.cpu_count() or 2) - 1))
atexit.register(PROCESS_POOL.shutdown)
...
async def full_search_pipeline(query: str, content_extractor: ContentExtractor):
