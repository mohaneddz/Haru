...
export default function TreeNode(props: { node: FileNode; level: number }) {
    ...
    return (
        <li> {/* CSS .tree li will apply */}
            <div
                class="tree-item cursor-pointer hover:bg-white/5 transition-colors duration-150"
                onClick={toggleOpen}
                // Removed inline padding-left style
            >
                {/* Chevron placeholder for consistent alignment */}
                <div style={{
                    width: '18px', // Adjust width as needed for chevron + spacing
                    height: '18px', // Match icon height for alignment
                    display: 'flex',
                    'align-items': 'center',
                    'justify-content': 'center',
                    'flex-shrink': 0
                }}>
                    {props.node.type === 'folder' && hasChildren && (
                        isOpen() ? (
            ...
            {props.node.type === 'folder' && hasChildren && isOpen() && (
                <ul class="transition-all duration-200 ease-in-out"> {/* CSS .tree ul will apply */}
