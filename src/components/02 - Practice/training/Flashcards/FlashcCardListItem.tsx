...
export default function FlashcCardListItem(props: Props) {
    return (
        <li class="p-3 bg-sidebar-light-1 rounded-md flex bg-sidebar justify-start items-start flex-col border-gray-400 border-1">
            <div class="flex justify-between w-full">
                <span class="text-accent text-xl center gap-2">
                    <Checkbox onChange={props.onCheck} selected={props.selected().includes(props.id || 0)} />
                    <p onClick={props.onEdit} class="cursor-pointer hover:underline">
                        {props.title || "Flashcard 1"}
                    </p>
                </span>
                <div class="flex gap-4">
                    <Play
                        class="text-gray-400 hover:text-accent transition-colors cursor-pointer"
                        onClick={() => props.onPlay?.(props.id || 0)}
                    />
                    <Trash2
            ...
            <span class="text-gray-400 text-sm pl-4">{props.description || "Because it's black and white, it has a classic look"}</span>
