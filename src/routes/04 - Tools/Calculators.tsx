...
export default function Calculators() {
  ...
  // Cleanup keyboard listeners on component unmount
  onCleanup(() => {
  ...
  const handleOutsideClick = (event: MouseEvent) => {
    ...
    const button = document.querySelector('.shortcuts-button'); // Reference the button
    if (
      modal && !modal.contains(event.target as Node) &&
      button && !button.contains(event.target as Node) // Exclude button clicks
    ) {
  ...
  return (
    <div class="h-full w-full bg-background text-text p-6 overflow-hidden">
      <div class="mx-16 flex flex-col gap-2 h-full"> {/* Increased max-width and added flex column gap */}
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mb-4">
          ...
          <div class="flex items-center gap-3 z-50">
            ...
            <button
              class="px-3 py-1 rounded-md text-xs bg-primary/20 text-primary hover:bg-primary/30 transition-colors flex items-center gap-1 shortcuts-button" // Add class for reference
        ...
        {/* Keyboard Help Modal */}
        {showKeyboardHelp() && (
          <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4 overflow-hidden">
            <div class="keyboard-help-modal bg-sidebar rounded-xl border border-border-light-1 p-6 max-w-4xl w-full max-h-[80vh] overflow-x-hidden">
        ...
        <div class="flex gap-6 h-full w-full overflow-hidden">
          ...
          {/* Main Calculator */}
          <div class="flex flex-col relative bg-sidebar rounded-xl border border-border-light-1 p-6 h-full w-full overflow-hidden">
            <Display
              displayValue={display}
